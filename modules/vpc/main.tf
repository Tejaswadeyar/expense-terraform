resource "aws_vpc" "main" {
  ...
  tags = {
    Name = "${var.env}-${var.project_name}-vpc"   #it shows dev-vpc
  }
}
...
resource "aws_route_table_association" "public" {
  count          = length(var.public_subnets_cidr)
  route_table_id = lookup(element(aws_route_table.public, count.index), "id", null) # alternate=aws_route_table.public[count.index].id
...
resource "aws_route_table_association" "web" {
  count          = length(var.web_subnets_cidr)
  route_table_id = lookup(element(aws_route_table.web, count.index), "id", null) # alternate=aws_route_table.private[count.index].id
...
resource "aws_route_table_association" "app" {
  count          = length(var.app_subnets_cidr)
  route_table_id = lookup(element(aws_route_table.app, count.index), "id", null) # alternate=aws_route_table.private[count.index].id
...
resource "aws_route_table_association" "db" {
  count          = length(var.db_subnets_cidr)
  route_table_id = lookup(element(aws_route_table.db, count.index), "id", null) # alternate=aws_route_table.private[count.index].id
